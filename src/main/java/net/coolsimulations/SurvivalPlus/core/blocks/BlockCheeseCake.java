            ...
            if(playerIn.getMainHandStack().getItem() == SPItems.paper_cup || playerIn.getOffHandStack().getItem() == SPItems.paper_cup) {
            	...
            	if(!playerIn.isCreative()) {
					...
					if(itemStackIn.getCount() == 1) {
						...
						{
							playerIn.setStackInHand(Hand.OFF_HAND, new ItemStack(SPItems.cheese_cupcake));
						}
						else
						{
							playerIn.setStackInHand(Hand.MAIN_HAND, new ItemStack(SPItems.cheese_cupcake));
						}
					} else if(itemStackIn.getCount() >= 2){
						itemStackIn.decrement(1);
						boolean flag = playerIn.inventory.insertStack(new ItemStack(SPItems.cheese_cupcake));
						if(!flag) {
