...
public class BlockCardboardLantern extends BlockCardboard implements IWaterLoggable {
	...
	public boolean canBlockStay(World worldIn, BlockPos pos, BlockState state)
	{
		...
		return ifluidstate.getFluid() == Fluids.WATER || (material == Material.ICE || material == Material.PACKED_ICE) || (iblockstate.getBlock() instanceof CauldronBlock && iblockstate.get(CauldronBlock.LEVEL) == 3) || (SPCompatibilityManager.isCavesAndCliffsBackportLoaded() && iblockstate.getBlock() instanceof AbstractCauldronBlock && iblockstate.getBlock() != CCBBlocks.LAVA_CAULDRON.get() && ((AbstractCauldronBlock) iblockstate.getBlock()).isFull(iblockstate));
	}
	...
	protected static boolean canPlaceBlock(World worldIn, BlockPos pos, Direction direction)
	{
		...
		if (direction == Direction.UP)
		{
			...
			return iblockstate.isSolidSide(worldIn, blockpos, direction.getOpposite()) && hasSolidSide(iblockstate, worldIn, blockpos, direction);
		} else {
	...
	protected boolean canFloat(World worldIn, BlockPos pos, Direction direction)
	{
		...
		return direction == Direction.UP && (iblockstate.getBlock() == Blocks.WATER || ((iblockstate.getMaterial() == Material.ICE || iblockstate.getMaterial() == Material.PACKED_ICE) && direction != Direction.DOWN) || (iblockstate.getBlock() instanceof CauldronBlock && iblockstate.get(CauldronBlock.LEVEL) == 3)  || (SPCompatibilityManager.isCavesAndCliffsBackportLoaded() && iblockstate.getBlock() instanceof AbstractCauldronBlock && iblockstate.getBlock() != CCBBlocks.LAVA_CAULDRON.get() && ((AbstractCauldronBlock) iblockstate.getBlock()).isFull(iblockstate)));
	}
