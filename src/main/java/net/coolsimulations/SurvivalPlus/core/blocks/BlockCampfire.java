...
public class BlockCampfire extends Block{
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		...
		igniteCampfire(worldIn, playerIn, state, pos, itemStackIn, Items.FLINT_AND_STEEL, SoundEvents.ITEM_FLINTANDSTEEL_USE, 1);
		igniteCampfire(worldIn, playerIn, state, pos, itemStackIn, Items.FIRE_CHARGE, SoundEvents.ITEM_FIRECHARGE_USE, ItemStack.EMPTY.getItem());
		igniteCampfire(worldIn, playerIn, state, pos, itemStackIn, Items.LAVA_BUCKET, SoundEvents.ITEM_BUCKET_EMPTY_LAVA, Items.BUCKET);
		if(SPCompatibilityManager.isNoTreePunchingLoaded())
			igniteCampfire(worldIn, playerIn, state, pos, itemStackIn, Item.REGISTRY.getObject(new ResourceLocation(SPCompatibilityManager.NO_TREE_PUNCHING_MODID, "fire_starter")), SoundEvents.ITEM_FLINTANDSTEEL_USE, 1);

		extinguishCampfire(worldIn, playerIn, state, pos, itemStackIn, Items.WATER_BUCKET, SoundEvents.BLOCK_LAVA_EXTINGUISH, Items.BUCKET);
		extinguishCampfire(worldIn, playerIn, state, pos, itemStackIn, water_bottle, SoundEvents.BLOCK_LAVA_EXTINGUISH, Items.GLASS_BOTTLE);
		extinguishCampfire(worldIn, playerIn, state, pos, itemStackIn, Items.BUCKET, SoundEvents.BLOCK_GRAVEL_STEP, SPItems.charcoal_bucket);
		if(SPCompatibilityManager.isAetherLegacyLoaded()) {
			Item skyroot_bucket = Item.REGISTRY.getObject(new ResourceLocation(SPCompatibilityManager.AETHER_LEGACY_MODID, "skyroot_bucket"));
			extinguishCampfire(worldIn, playerIn, state, pos, itemStackIn, new ItemStack(skyroot_bucket, 1, 1).getItem(), SoundEvents.BLOCK_LAVA_EXTINGUISH, skyroot_bucket);
		}

		if(fluid != null) {
			if(!worldIn.isRemote && fluid == FluidRegistry.getFluidName(FluidRegistry.LAVA) && state.getValue(BURNING) == false) {
				igniteCampfire(worldIn, playerIn, state, pos, SoundEvents.ITEM_BUCKET_EMPTY_LAVA);
				...
				extinguishCampfire(worldIn, playerIn, state, pos, SoundEvents.BLOCK_LAVA_EXTINGUISH);
				if(!playerIn.capabilities.isCreativeMode){
					itemStackIn.getTagCompound().removeTag("Fluid");
					itemStackIn.getTagCompound().removeTag("Amount");
				}
	...
	protected void igniteCampfire(World worldIn, EntityPlayer playerIn, IBlockState state, BlockPos pos, ItemStack itemStackIn, Item item, SoundEvent sound, int damage) {
		if(!worldIn.isRemote && itemStackIn.getItem() == item && state.getValue(BURNING) == false && !canDie(worldIn, pos)) {
			igniteCampfire(worldIn, playerIn, state, pos, sound);
			itemStackIn.damageItem(damage, playerIn);
		}
	}

	protected void igniteCampfire(World worldIn, EntityPlayer playerIn, IBlockState state, BlockPos pos, ItemStack itemStackIn, Item item, SoundEvent sound, Item replace) {
		if(!worldIn.isRemote && itemStackIn.getItem() == item && state.getValue(BURNING) == false && !canDie(worldIn, pos)) {
			igniteCampfire(worldIn, playerIn, state, pos, sound);
			if(!playerIn.capabilities.isCreativeMode){
				if (ItemStack.areItemStacksEqual(playerIn.getHeldItemOffhand(), itemStackIn))
				{
					playerIn.setHeldItem(EnumHand.OFF_HAND, new ItemStack(replace));
				}
				else
				{
					playerIn.setHeldItem(EnumHand.MAIN_HAND, new ItemStack(replace));
				}
			}
		}
	}
	...
	protected void extinguishCampfire(World worldIn, EntityPlayer playerIn, IBlockState state, BlockPos pos, ItemStack itemStackIn, Item item, SoundEvent sound, Item replace) {
		if(!worldIn.isRemote && itemStackIn.getItem() == item && state.getValue(BURNING) == true) {
			extinguishCampfire(worldIn, playerIn, state, pos, sound);
			if(!playerIn.capabilities.isCreativeMode){
				if(itemStackIn.getCount() == 1) {
					...
					boolean flag = playerIn.inventory.addItemStackToInventory(new ItemStack(replace));
					if(!flag) {
						playerIn.dropItem(new ItemStack(replace), false);
					}
