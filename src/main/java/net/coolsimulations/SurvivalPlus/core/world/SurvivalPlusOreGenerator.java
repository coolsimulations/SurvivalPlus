		...
		if (category != BiomeCategory.NETHER && category != BiomeCategory.THEEND) {
			if(!SPConfig.disableTinOreGen.get()) {
				if(name.toString().equals("minecraft:dripstone_caves")) {
					gen.addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, Feature.ORE.configured(new OreConfiguration(ORE_TIN_TARGET_LIST, 20)).placed(commonOrePlacement(16, HeightRangePlacement.triangle(VerticalAnchor.absolute(-16), VerticalAnchor.absolute(112)))));
				} else {
					gen.addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, Feature.ORE.configured(new OreConfiguration(ORE_TIN_TARGET_LIST, 10)).placed(commonOrePlacement(16, HeightRangePlacement.triangle(VerticalAnchor.absolute(-16), VerticalAnchor.absolute(112)))));
				}
			}
			if(!SPConfig.disableTitaniumOreGen.get()) {
				gen.addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, Feature.ORE.configured(new OreConfiguration(ORE_TITANIUM_TARGET_LIST, 4)).placed(commonOrePlacement(10, HeightRangePlacement.triangle(VerticalAnchor.absolute(-64), VerticalAnchor.absolute(25)))));
				gen.addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, Feature.ORE.configured(new OreConfiguration(ORE_TITANIUM_TARGET_LIST, 8)).placed(rareOrePlacement(8, HeightRangePlacement.triangle(VerticalAnchor.aboveBottom(-62), VerticalAnchor.aboveBottom(62)))));
			}
		}
	}
