...
public class CommandSleep {
				...
				if (!sender.getLevel().isDay()) {
					if (entityplayer == sender.getEntity()) {
						if (sender.getEntity() instanceof Player && ((Player) sender.getEntity()).isSleeping()) {
							...
							if (dimension == null) {
								MutableComponent sleep = Component.translatable("sp.commands.sleep.display3", new Object[]{sender.getDisplayName(), getDimensionName(sender.getLevel().dimension().location())});
	...
	private static String getDimensionName(ResourceLocation location) {
		return StringUtils.capitaliseAllWords(location.getPath().replaceAll("_", " "));
	}
}
